...
function InitGrid() {
	$('#costList').datagrid({
		url:'/HPOA/reimburse/getUiGridCost.json', //指向后台的Action来获取当前菜单的信息的Json格式的数据
		title: '费用报销单',
		iconCls: 'icon-view',
      //  height: 300,
        ...
        toolbar: [{
            ...
            handler: function () {
            	...
            	location.href='/HPOA/reimburse/goCreateCost';
            }
        },
        '-', {
            id: 'btnDelete',
            ...
            handler: function () {
                //Delete();//实现直接删除数据的方法
            	var checkrow=$("#costList").datagrid("getSelections");
        ...
        '-', {
            ...
            handler: function () {
                //实现刷新栏目中的数据
                $("#costList").datagrid("reload");
            }
        }],
		onDblClickRow: function (rowIndex, rowData) {
			$('#costList').datagrid('uncheckAll');
			$('#costList').datagrid('checkRow', rowIndex);
		}
	});
	//添加操作行
	function formatOper(val,row,index){
		setTopLink="<a href='javascript:void(0);' onclick='";
		setTopLink+="findCost(\""+row.reimburseid+"\")'>查看</a>&nbsp;&nbsp;&nbsp;&nbsp;";
		return setTopLink;
	}
}

function findCost(reimburseid){
	location.href="/HPOA/reimburse/findCost?reimburseId="+reimburseid;
}