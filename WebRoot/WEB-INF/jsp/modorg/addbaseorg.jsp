...
<html>
  <head>
	...
	<!-- addbaseorg.js -->
	<script type="text/javascript" src="<%=path%>/js/modorg/addbaseorg.js"></script>
	<style>
		*{padding:0;margin:0}
  ...
  <body>
    <input type="hidden" id="orgid" value="${soi.orgId }">
    <table>
    	<tr>
    	...
    	<tr>
    		<td><label>所属主部门</label></td>
    		<td>
    			<select id="orgdep" style="width:120px;" onchange="loadstaff()" value="${depid}">
			 		<option value="0">请选择</option>			 	
			 		<c:forEach items="${depList}" var="dep"> 
			 			<c:if test="${dep.itemsid==depid}">
			 				<option value='${dep.itemsid}' selected>${dep.itemsname}</option>
			 			</c:if>
			 			<c:if test="${dep.itemsid!=soi.orgDepid}">
			 				<option value='${dep.itemsid}'>${dep.itemsname}</option>
			 			</c:if> 
 			 		</c:forEach> 
			 	</select>
			</td>
