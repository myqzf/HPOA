...
public class Criterion {
     ...
     * 
     * @Title: convertToSql 
     * @Description: TODO(把条件转成sql语句)
     * @param criteria
     * @return String
     *
     */
    public static String convertToSql2(List<Criterion> criteria) {  
        String criteriaString = "";  
        StringBuilder sb = new StringBuilder();  
        for (Criterion criterion : criteria) {  
            String prefix = criterion.getFieldPrefix();  
            switch (criterion.getCriterionType()) {  
            case EQUAL:  
                sb.append(prefix + criterion.getField() + "='"+criterion.getValue()+"' and ");  
                break;  
            case LIKE:  
                sb.append(prefix + criterion.getField() + " like '"+criterion.getValue()+"' and ");  
                break;  
                  
            case NOT_LIKE:  
                sb.append(prefix + criterion.getField() + " not like '"+criterion.getValue()+"' and ");  
                break;  
            case COMPARE:  
                CompareType compareType = ((CompareCriterion) criterion)  
                        .getCompareType();  
